...
\begin{itemize}[leftmargin=*, itemsep=4pt]
    ...
    \item \textbf{Results:} Random Forest (100 trees, max depth 10) achieved
          ...
          minority class. A surrogate decision tree (98.3\% fidelity) distills
          the ensemble into actionable rules: the ideal customer has
          \textbf{Income $\geq$ \$117K} with a \textbf{graduate+ degree}, or
          \textbf{Income $\geq$ \$105K} with a \textbf{family of 3+}.
    ...
    \item \textbf{Recommendations:} Deploy the Random Forest model for targeted
          campaigns. The surrogate model provides directly implementable rules:
          target customers with Income $\geq$ \$117K and a graduate or advanced
          degree (primary segment), or Income $\geq$ \$105K with family size
          $\geq$ 3 (secondary segment). This targeted approach can reduce
          marketing effort by approximately 10$\times$ compared to the blanket
          strategy.
\end{itemize}
...
\subsection{Surrogate Model --- Interpretable ICP Rules}

To translate the Random Forest's predictions into actionable decision rules, a
\textbf{surrogate model} was fitted: a shallow decision tree (max depth~3)
trained on the Random Forest's own predictions rather than the original labels.
This distills the ensemble's learned decision boundaries into a small set of
human-readable rules with exact feature thresholds.

\begin{table}[H]
\centering
\caption{Surrogate model performance}
\label{tab:surrogate}
\begin{tabular}{@{} l r @{}}
\toprule
\textbf{Metric} & \textbf{Value} \\
\midrule
Fidelity to Random Forest & 98.30\% \\
Accuracy (vs true labels) & 97.80\% \\
Depth / Leaves            & 4 / 7 \\
\bottomrule
\end{tabular}
\end{table}

The surrogate tree agrees with the Random Forest on 98.3\% of test predictions,
confirming it is a faithful approximation. Its 97.8\% accuracy against true
labels shows that the interpretability cost is minimal (1.3 percentage points
below the full RF).

The extracted rules for the positive class (\textit{Accepted}) define the
\textbf{Ideal Customer Profile (ICP)} with exact thresholds:

\begin{table}[H]
\centering
\caption{Surrogate model ICP rules for personal loan acceptance}
\label{tab:icp_rules}
\begin{tabular}{@{} c p{8.5cm} r @{}}
\toprule
\textbf{Rule} & \textbf{Conditions} & \textbf{Coverage} \\
\midrule
1 & Income $\geq$ \$117K \textbf{and} Education $\geq$ Graduate & 6.3\% \\
2 & Income $\geq$ \$105K \textbf{and} Education = Undergrad \textbf{and} Family $\geq$ 3 & 1.7\% \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Interpretation:} The primary ICP segment comprises high-income customers
(\$117K+) with graduate or advanced degrees---this single rule covers 6.3\% of
the training population and captures the majority of loan acceptors. A secondary
segment captures high-income undergraduates (\$105K+) with larger families
(3--4 members), adding 1.7\% coverage. Together, these two rules provide a
concrete, actionable targeting strategy with specific thresholds that marketing
teams can directly implement.
...
The surrogate model (Section~7.4) distills the Random Forest's predictions into
concrete ICP rules with exact thresholds. Combined with permutation feature
importance, this identifies two primary customer segments:

\begin{enumerate}[leftmargin=*, itemsep=4pt]
    \item \textbf{High-income graduates (\$117K+, Education $\geq$ 2):}
          The dominant segment. Customers with annual income at or above \$117K
          and a graduate or advanced degree have the highest loan acceptance
          probability. This rule alone covers 6.3\% of the customer base and
          captures the majority of acceptors.

    \item \textbf{High-income undergraduates with large families (\$105K+,
          Family $\geq$ 3):} A secondary segment of high earners without
          advanced degrees but with family sizes of 3--4. This captures an
          additional 1.7\% of customers.
\end{enumerate}

Additional signals from the permutation importance analysis reinforce the
targeting strategy:
\begin{itemize}[itemsep=2pt]
    \item \textbf{Income} is by far the most predictive feature (0.181 mean
          accuracy decrease), confirming its role as the primary filter.
    \item \textbf{CCAvg} and \textbf{CD Account} provide secondary lift---customers
          with high credit card spending or existing CD relationships show
...
\begin{enumerate}[leftmargin=*, itemsep=4pt]
    ...
    \item \textbf{Target two specific customer segments} identified by the
          surrogate model: (a)~customers with Income $\geq$ \$117K and a
          graduate or advanced degree, and (b)~customers with Income $\geq$
          \$105K, undergraduate education, and family size $\geq$ 3. These
          rules can be directly implemented as campaign filters.
