...
fn main() {
    ...
    println!(
        "  After ToTensor::imagenet(): shape {:?}, dtype f32",
        tensor.shape.dims()
    );
    ...
    println!("  Embedding C (photo of a truck): {embed_c:?}\n");

    let sim_ab = cosine_similarity(&embed_a, &embed_b);
    let sim_ac = cosine_similarity(&embed_a, &embed_c);
    println!("  cosine_similarity(A, B) = {sim_ab:.4}  <- similar images, high score");
    println!("  cosine_similarity(A, C) = {sim_ac:.4}  <- different images, low score\n");
